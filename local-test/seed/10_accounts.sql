...
--   CompanyEntity -> 'WSG', RegionId/BusinessUnitId/PillarId/
...
IF NOT EXISTS (SELECT 1 FROM cp.Account WHERE AccountId = 'MsgIntTest')
    INSERT cp.Account (AccountUid, AccountId, AccountName, CustomerType, Product_SMS)
...
IF NOT EXISTS (SELECT 1 FROM ms.SubAccount WHERE SubAccountId = 'MsgIntTest_1')
    INSERT ms.SubAccount (SubAccountUid, SubAccountId, AccountUid, Active, Product_SMS, OmnishieldStatusId)
